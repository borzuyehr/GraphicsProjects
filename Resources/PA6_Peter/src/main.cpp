...
bool loadOBJ(char* obj, std::vector<Vertex>& geometry, char* tex);
void Bind();


//--Resource management
bool initialize(char* fileName, char* textureName);
void cleanUp();
...
int main(int argc, char **argv)
{
    ...
    if (argc > 2)//if correct name that exists
        {
        strcpy (fileName, argv[1]); //copy file name into array
        strcpy(textureName, argv[2]); // copy the texture name into array
        }
    else
        {
    ...
    bool init = initialize(fileName, textureName);
    if(init)
    {
...
bool initialize(char* fileName, char* textureName)
{
    ...
    bool geometryLoadedCorrectly = loadOBJ (fileName, geometry, textureName); 
    ...
    // add the sampler or it will SEG FAULT

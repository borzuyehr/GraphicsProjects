...
bool loadOBJ(char * obj, std::vector<Vertex>& geometry)
{
    ...
    // otherwise, start looping through file
    while (1)
        {
        numOfItemsInFile = fscanf (file, "%s", line);

        //if we reach end of file leave loop
        if (numOfItemsInFile <=0)
            {
            break;
            }
        //otherwise keep going
            ...
            if (strcmp (line, "v") == 0)
                ...
                fscanf (file, "%f %f %f\n", &(temporaryVertex.position[0]),
                                            &(temporaryVertex.position[1]),
                                            &(temporaryVertex.position[2]));

                temporaryVertex.color[0] =0.0f;
                temporaryVertex.color[1] =1.0f;
                temporaryVertex.color[2] =0.0f;

                temp.push_back(temporaryVertex);
                }
            else if (strcmp (line, "f") == 0)
                {
                int vIndex [3];
                int hits = fscanf (file, "%d %d %d\n", &vIndex[0],
                                            &vIndex[1],
                                            &vIndex[2]);
                ...
                index.push_back (vIndex[0]);
                index.push_back (vIndex[1]);
                index.push_back (vIndex[2]);
                }
            else
                {
                fgets(extra, 128, file);
                }
            }
        }
    ...
    for(unsigned int i = 0; i< index.size (); i++)
        {
        unsigned int vertexInd = index[i];
        Vertex V = temp [vertexInd - 1];
